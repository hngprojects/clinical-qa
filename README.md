...
This Postman collection contains a complete regression test suite for the Clinsight API MVP, covering all critical authentication and core feature endpoints.
...
1. AUTH - SIGNUP (POST /api/v1/auth/signup)
...
Empty Credentials	4xx	Empty email and password
Unverified Email Account	404	Unverified account login
3. AUTH - FORGOT PASSWORD (POST /api/v1/auth/forgot-password)
Test	Expected Status	Description
Registered email	200	Valid existing email
 Non-registered email	200	Anti-enumeration (still 200)
 Invalid email format	422	Malformed email
Empty body	422	No data sent
4. AUTH - LOGOUT (POST /api/v1/auth/logout)
Test	Expected Status	Description
Valid token	200	Successful logout
No token	401	Missing Authorization header
Revoked token	401	Already used token
5. GUEST SESSIONS
...
Empty string fingerprint	200/201	Handles gracefully
Very long fingerprint	201	Stress test
Get Guest Session (GET /api/v1/guest-session/me)
Test	Expected Status	Description
...
6. LAB RESULTS - UPLOAD (POST /api/v1/upload)
Test	Expected Status	Description
Guest user with valid file	201	Uploads image/file
...
7. AI INTERPRETATIONS (GET /api/v1/cases/{case_id}/interpretations)
Test	Expected Status	Description
Invalid case_id format	422	Non-UUID format
Case ID not found	200/404	Empty or 404 response
Get AI Interpretations	-	Retrieves interpretations for created case
🚀 Running the Collection
Prerequisites
Postman (latest version) or Newman (CLI)

Test file: lab_result.jpeg must exist in the working directory for upload tests

Via Postman Desktop
Import the regression.json collection

Set up your environment variables

Run the collection in order (folders are sequenced)
...
newman run regression.json \
  ...
  --reporters cli,json \
  --report-json-export test-results.json
...
Notes
Rate Limiting: Some tests accept 429 responses (rate limiting) as valid outcomes

Session Persistence: Guest session IDs are automatically stored and reused

File Upload: Requires lab_result.jpeg in the working directory

Anti-Enumeration: Forgot password endpoint returns 200 even for non-existent emails (security feature)
...
Troubleshooting
Issue	Solution
401 Unauthorized	Check test_email/test_password are valid
422 Validation Error	Verify email/password format requirements
Upload test fails	Ensure lab_result.jpeg exists in the directory
Guest session fails	Run "Create Guest Session" first to set the variable
